...
class ResNetBase(nn.Module):
    BLOCK = None   # 不是None，被覆盖为MinkowskiEngine中的BasicBlock类
    LAYERS = ()    # 被子类的LAYERS覆盖为(1, 1, 1, 1, 1, 1, 1, 1), 表示要使用多少个BLOCK自网络拼接
    INIT_DIM = 64  # 输入的特征维度被覆盖为32
    PLANES = (64, 128, 256, 512)  # 各层的特征维度，被MinkUNetBase子类覆盖为(32, 64, 128, 256, 192, 192, 192, 192)
    ...
    def __init__(self, in_channels, out_channels, D=3):
        ...
        self.network_initialization(in_channels, out_channels, D)  # in_channels=3，out_channels=512, D=3
        self.weight_initialization()

    def network_initialization(self, in_channels, out_channels, D): # in_channels=3，out_channels=512, D=3

        self.inplanes = self.INIT_DIM   # 32
        # 在PyTorch中，ReLU函数默认情况下不会对原始输入张量进行修改，即不会在原地（inplace）进行操作。但是，我们可以通过将inplace参数设置为True来开启原地操作，从而直接修改输入张量，而不需要额外的存储空间来存储中间结果。
        ...
        # self.PLANES = (32, 64, 128, 256, 192, 192, 192, 192)
        # self.LAYERS = (1, 1, 1, 1, 1, 1, 1, 1)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilation=1, bn_momentum=0.1):
        downsample = None
        # block是BasicBlock，BasicBlock的expansion=1
        ...
        for i in range(1, blocks):   # blocks都等于1，因此这个区间是空的，即没有任何元素，下面的语句不执行
            layers.append(
                block(
