...
def process_grasp_labels(end_points):
    ...
    end_points['batch_grasp_point'] = batch_grasp_points   # (B, Ns, 3)
    end_points['batch_grasp_view_rot'] = batch_grasp_views_rot  # (B, Ns, V, 3, 3)
    end_points['batch_grasp_score'] = batch_grasp_scores    # (B, Ns, V, A, D)
    end_points['batch_grasp_width'] = batch_grasp_widths    # (B, Ns, V, A, D)
...
def match_grasp_view_and_label(end_points):
    ...
    top_view_inds = end_points['grasp_top_view_inds']  # (B, Ns), 每个点对应的最优抓取方向的索引
    template_views_rot = end_points['batch_grasp_view_rot']  # (B, Ns, V, 3, 3)，每个点的每种抓取方向对应的从该点的理想坐标系转换到场景坐标系的旋转矩阵
    grasp_scores = end_points['batch_grasp_score']  # (B, Ns, V, A, D), 每个点每种抓取方向，每种旋转角度，每种抓取距离对应的抓取分数
    grasp_widths = end_points['batch_grasp_width']  # (B, Ns, V, A, D), 每个点每种抓取方向，每种旋转角度，每种抓取距离对应的抓取宽度值

    # 
    B, Ns, V, A, D = grasp_scores.size()
    top_view_inds_ = top_view_inds.view(B, Ns, 1, 1, 1).expand(-1, -1, -1, 3, 3)  # 把每个点最优抓取方向的索引扩充到(B, Ns, 1, 3, 3)
    top_template_views_rot = torch.gather(template_views_rot, 2, top_view_inds_).squeeze(2) # (B, Ns, 3, 3)
    top_view_inds_ = top_view_inds.view(B, Ns, 1, 1, 1).expand(-1, -1, -1, A, D)  # (B, Ns, 1, A, D)
    top_view_grasp_scores = torch.gather(grasp_scores, 2, top_view_inds_).squeeze(2)  # (B, Ns, A, D)
