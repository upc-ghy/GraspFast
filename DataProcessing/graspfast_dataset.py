...
class GraspFastDataset(Dataset):
    def __init__(self, root, grasp_labels=None, camera='kinect', split='train', num_points=20000,
        ...
        for x in tqdm(self.sceneIds, desc='Loading data path and collision labels...'):
            for img_num in range(256):
                ...
                self.graspfastpath.append(os.path.join(root, 'graspfast', x, camera, str(img_num).zfill(4) + '.npy'))
    ...
    def get_data_label(self, index):
        ...
        seg = np.array(Image.open(self.labelpath[index]))
        meta = scio.loadmat(self.metapath[index])
        graspfast = np.load(self.graspfastpath[index])
        scene = self.scenename[index]
        try:
        ...
        graspfast_sampled = graspfast[idxs]
        ...
        ret_dict = {'point_clouds': cloud_sampled.astype(np.float32),
                    'coors': cloud_sampled.astype(np.float32) / self.voxel_size,
                    'feats': np.ones_like(cloud_sampled).astype(np.float32),
                    'graspfast_label': graspfast_sampled.astype(np.float32),
